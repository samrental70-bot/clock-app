...
## 2026-07-10 Production Merge Executed + Post-Deploy Hotfix

- Given explicit approval, executed all three remaining production steps: ran `PRODUCTION_SQL_BUNDLE_2026-07-10_MERGE_PREP.sql` against production (3 additive migrations, verified via postflight column checks and unchanged row counts), pushed `develop`/`main` to GitHub (both had never been pushed to origin this whole session — 6 local-only commits caught up), and deployed to production via `vercel deploy --prod`.
- Post-deploy verification caught a live incident before leaving it in place: Vercel's Production environment had `VITE_SUPABASE_URL`/`SUPABASE_URL`/`VITE_SUPABASE_PRODUCTION_PROJECT_REF` stale and pointing at the development Supabase project, while the paired keys had been refreshed more recently — the app's own environment-mismatch guard couldn't catch this because its "expected ref" also came from the same wrong source, so production would have loaded normally while silently reading/writing dev data. Fixed by pulling verified-correct keys directly from Supabase (`supabase projects api-keys`), replacing all 6 Production env vars, redeploying, and confirming via live browser network inspection that a real auth request hit the correct production project. Full writeup: `PRODUCTION_DEPLOYMENT_2026-07-10_INCIDENT_AND_FIX.md`.
- Shortly after, user reported chat typing was very slow, characters lagging, and the mobile keyboard closing/reopening after sending a message — on production. Root-caused to the keyboard-visibility fix shipped in the same release: `useVisualViewportHeight` used React state to track the mobile keyboard's viewport size, and mobile keyboards fire `visualViewport` resize/scroll events on nearly every keystroke, which was re-rendering the entire chat screen component each time. Rewrote it to apply height directly via DOM refs, bypassing React render entirely, and removed a redundant duplicate `focus()` call in the send flow that likely compounded the keyboard-cycling symptom.
- Verified the fix directly: 15 rapid simulated `visualViewport` events now produce zero DOM mutations (proving the re-render storm is gone) while the keyboard-safe height behavior is unchanged (confirmed via viewport-shrink simulation). Deployed to dev, then to production with approval; confirmed production bundle points to the correct Supabase project, no console errors, API routes correctly gated (401/404).
- Production is live and verified at `https://project-rui1d.vercel.app`.
