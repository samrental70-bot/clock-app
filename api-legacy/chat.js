...
async function fetchActiveChatListItems(supabase, { companyId, listId }) {
  const { data, error } = await supabase
    ...
    .select("id, list_id, item_number, text, is_done, completed_at, completed_by, created_by, created_at, updated_at, deleted_at, parent_item_id, item_level, child_order, sort_order")
...
async function addChatListItem(supabase, { companyId, callerId, listId, text, parentItemId }) {
  ...
  const { data: item, error } = await supabase
    ...
    .select("id, item_number, parent_item_id, item_level, child_order, sort_order")
    .single();
  if (error) throw error;
  ...
  return { id: item.id, item_number: item.item_number, parent_item_id: item.parent_item_id || null, item_level: item.item_level || 0 };
}
...
async function getChatListItemForMember(supabase, { companyId, itemId, callerId }) {
  ...
  const { data: item, error } = await supabase
    .from("chat_list_items")
    .select("id, company_id, conversation_id, list_id, text, is_done, deleted_at, parent_item_id, item_level, child_order, item_number, sort_order")
    .eq("company_id", companyId)
    .eq("id", id)
    .maybeSingle();
  if (error) throw error;
